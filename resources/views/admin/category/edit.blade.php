            ...
            <form action="{{ route('admin#categoryUpdate',$data->category_id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="categoryName" id="" class="form-control @error('categoryName')
                        is-invalid
                    @enderror" placeholder="Ender category Name" value="{{ $data->title }}">
                    @error('categoryName')
                    ...
                    <textarea class="form-control @error('description')
                        is-invalid
                    @enderror" cols="30" rows="5" name="description" placeholder="Ender Description">{{ $data->description }}</textarea>
                    @error('description')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                    <button class="btn btn-primary text-white mt-3" type="submit">Update</button>
                    <a href="{{ route('admin#category') }}">
                        <button class="btn btn-dark text-white mt-3" type="button">Create</button>
                    </a>
                </div>
            </form>
        </div>
